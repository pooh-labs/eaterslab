...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            losing_conn_ids = CameraEvent.objects.values('camera_id').annotate(last_event=Max('timestamp')).filter(pk__in=set(online_ids), last_event__lt=time_threshold).values_list('camera_id_id', flat=True)
